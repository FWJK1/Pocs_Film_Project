...
def get_NER_all(data, chunksize=40, begin=0, length=None, output_file=None):
    ...
    if not output_file:
        output_file = f'{root}/Data/2020_trope_data/Scraped_Data/NER_parsed_reviews.csv'

    if not length:
        length = len(df)

    print(f"NER cleaning reviews for movies from {begin} to {length} with chunksize={chunksize}")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=20) as executor:
        for start in range(begin, length, chunksize):
            ...
            chunk['NER_cleaned_reviews'] = [[] for _ in range(len(chunk))]  # Initialize with empty lists


            # Submit tasks for fetching comments and budget separately
            futures = {
                                index: executor.submit(
                                    replace_actors_with_placeholder,
                                    review_list=row['reviews_extracted'],  # equivalent to 'reviews'
            ...
            for index, future in futures.items():
                ner_reviews = future.result()

                # Directly assign the actors and roles to the correct index in the chunk
                chunk.at[index, 'NER_cleaned_reviews'] = ner_reviews ## note we use .at instead of .loc
    ...
    print(f"Cleaned all NER reviews from {begin} to {length} with chunksize={chunksize}")

if __name__ == '__main__':
    root = find_repo_root(os.getcwd())
    df = pd.read_csv(f"{root}/Data/2020_trope_data/Scraped_Data/merged_NER_scraped_data.csv", index_col=0)
    get_NER_all(df, begin=10600)